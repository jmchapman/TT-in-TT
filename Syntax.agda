...
data _≡⁺_ : forall {Γ Γ'} -> Ty Γ -> Ty Γ' -> Set 
data _≡ˢ_ : forall {Γ Γ' Δ Δ'} -> Sub Γ Δ -> Sub Γ' Δ' -> Set
data Tm : forall Γ -> Ty Γ -> Set 
data _≡_ : forall {Γ Γ' σ σ'} -> Tm Γ σ -> Tm Γ' σ' -> Set 
...
data _≡Con_ where
  congε : ε ≡Con ε
  cong, : forall {Γ Γ' σ σ'} -> Γ ≡Con Γ' -> σ ≡⁺ σ' -> (Γ , σ) ≡Con (Γ' , σ')
...
data _≡⁺_ where
  -- Setoid boilerplate
  coh⁺  : forall {Γ}{Δ}(σ : Ty Γ)(p : Γ ≡Con Δ) -> coe⁺ σ p ≡⁺ σ
  ...
  congU : forall {Γ}{Γ'} -> Γ ≡Con Γ' -> U {Γ} ≡⁺ U {Γ'}
  congEl : forall {Γ Γ'}{t : Tm Γ U}{t' : Tm Γ' U} -> Γ ≡Con Γ' -> t ≡ t' -> 
           El t ≡⁺ El t'
...
data _≡ˢ_ where
  ...
  cong• : forall {B B' Γ Γ' Δ Δ'}{ts : Sub Γ Δ}{us : Sub B Γ} ->
          ...
          ts • us ≡ˢ ts' • us'
  congid : forall {Γ Γ'} -> Γ ≡Con Γ' -> id {Γ} ≡ˢ id {Γ'}
